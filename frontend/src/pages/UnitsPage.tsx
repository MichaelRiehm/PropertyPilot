...
export default function UnitsPage() {
  ...
  const allUnits = unitsQuery.data?.data ?? [];
  ...
  const filteredUnits = useMemo(() => {
    if (filterPropertyId === 'all') return allUnits;
    return allUnits.filter((u) => u.propertyId === filterPropertyId);
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && properties.length > 0 && filteredUnits.length === 0 && (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-slate-100">
            <DoorOpen className="h-6 w-6 text-slate-500" aria-hidden="true" />
          </div>
